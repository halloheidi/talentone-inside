-- Migration 008: Garantie-Modell, Einstellungen (talentone_hires) und
-- Skip-Log für den monatlichen Rechnungslauf.
--
-- Grundprinzip: keine Mindestlaufzeit. Servicepauschale nur, solange die
-- Garantiefrist läuft ODER eine Einstellung erfasst ist. Nach Frist ohne
-- Einstellung: automatisch servicefrei. TalentOne max 1 servicefreier
-- Monat, dann manuelle Reaktivierung.
...
-- ─── talentone_hires (mehrere Einstellungen pro Angebot möglich) ───
...
-- ─── talentone_billing_skip_log (übersprungene monatliche Läufe) ───
CREATE TABLE IF NOT EXISTS public.talentone_billing_skip_log (
  ...
  reason                 text NOT NULL CHECK (reason IN (
    'guarantee_no_hire',            -- Frist abgelaufen, keine Einstellung
    'talentone_max_month_reached',  -- TalentOne: 2. servicefreier Monat blockiert
    'service_waived_override',      -- manueller Kulanz-Schalter
    'campaign_ended',               -- Abo beendet
    'other'
  )),
...
-- ─── Neue Textbausteine (Einstellungs-Mails) ───
-- hire_email_subject gemeinsam; die drei Bodies: first/progress/complete
...
-- ─── Textbausteine UPDATE (BESTEHENDE Rows anpassen!) ───
-- Merge-Tag {{garantie_frist}} in beiden guarantee-Templates.
...
-- N&W guarantee mit "solange die Zusammenarbeit besteht, bis die erste Einstellung erfolgreich abgeschlossen ist"
UPDATE public.talentone_offer_templates
...
-- N&W payment_terms: Mindestlaufzeits-Satz ersatzlos gestrichen
UPDATE public.talentone_offer_templates
