-- Migration 007: Abrechnungs-Workflow (Phase 5).
--   - talentone_invoices  (jede erzeugte Rechnung)
--   - talentone_offers    (+recurring-Referenz, +Ende der monatlichen Abrechnung)
--   - talentone_kunden    (+campaign_payment_status, +paypal_enabled)
--   - talentone_ad_budget_history (Änderungshistorie Werbebudget)
...
CREATE TABLE IF NOT EXISTS public.talentone_invoices (
  ...
  paypal_reference       text,       -- Ref-String für den PayPal-Webhook-Match
...
-- Angebot: Recurring-Referenz + Ende der monatlichen Abrechnung
...
-- Kunde: Kampagnen-Zahlungsstatus + PayPal-Default
...
-- Werbebudget-Historie (nur TalentOne relevant, aber generisch je Angebot)
