...
export async function sendFormularEinladung({ to, ansprechpartner, formularUrl, customText }) {
...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;max-width:600px;width:100%;">
  <tr><td style="background:#0a0a0a;padding:24px 32px;">
    <span style="font-size:20px;font-weight:700;color:#ffffff;letter-spacing:-0.02em;">Talent<span style="color:#d4ff00;">One</span></span>
  </td></tr>
  <tr><td style="padding:28px 32px 8px;">
    <p style="font-size:15px;line-height:1.55;color:#0a0a0a;margin:0 0 14px;">Hallo ${escape(grußname)},</p>
    <p style="font-size:15px;line-height:1.6;color:#2a2a2a;margin:0 0 18px;">${escape(intro).replace(/\n/g, '<br>')}</p>
  ...
  <tr><td align="center" style="padding:0 32px 28px;">
    <a href="${formularUrl}" style="display:inline-block;background:#0a0a0a;color:#d4ff00;text-decoration:none;font-weight:700;font-size:15px;padding:14px 28px;border-radius:100px;letter-spacing:0.02em;">→ Formular ausfüllen</a>
  ...
  const response = await fetch(RESEND_API, {
    ...
    body: JSON.stringify({
      from: FROM, to, reply_to: 'hallo@talent-one.de',
...
/* ─────────────────── Mitarbeiter-Benachrichtigung bei Formular-Eingang ─────────────────── */
